...
* <2024-10-22 周二> 第 11 页

#+begin_quote
在虚拟继承的情况下， ~base class~ 不管在继承串链中被派生（ ~derived~ ）多少次，永远只会存在一个实体（称为 ~subobject~ ）。
#+end_quote

~subobject~ 会在文章后续内容中经常出现，有两个注意点：

1. 虚拟继承情况
2. ~subobject~ 指的是 ~base class~

* <2024-10-22 周二> 第 25 页

#+begin_quote
在 ~c++~ ，多态只存在于一个个的 ~public class~ 体系中。 ~nonpublic~ 的派生行为以及类型 ~void*~ 的指针可以说是多态，但它们并不有被语言明白地支持，也就是说它们由程序员通过明白的转型操作来管理（你或许可以说它们并不是多态对象的一线选手）。
#+end_quote

这里我的理解是：

1. ~public~ 体系下，父类对于子类都是公开的，子类完全清楚父类有几把刷子，就像类的使用者一样，也是可以完全看到的。
2. 而 ~nonpublic~ 的派生行为和 ~void*~ 的指针，由于封装的关系以及类型无法得知，对于类的使用者来说，完全看不到，所以需要通过明确的转型来管理，这似乎为多态的使用增加了一点儿麻烦。

<2024-10-22 周二> 第 34 页

~OB~ 和 ~OO~ 分别表示什么：
1. ~OB~ （ ~Object-Based~ ）是一种以对象为基础的数据抽象和封装方式。它主要关注数据的直接表示和操作，不涉及多态等复杂机制，因此相对于面向对象设计（ ~OO~ ）来说，基于对象的设计通常具有更高的执行效率和更紧凑的空间占用。即 ~ADT~ （ ~abstract data type~ ）。
2. ~OO~ （ ~Object-Oriented~ ）则是一种更为复杂和灵活的设计方法。它支持封装、继承和多态等特性，允许开发者通过对象来模拟现实世界中的关系和行为。
